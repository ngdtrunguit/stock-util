...
- If `refresh_agent=false` and the configured agent is missing, the workflow now attempts fallback reprovisioning automatically. When `agent_model_deployment` (or `AZURE_AI_AGENT_MODEL_DEPLOYMENT`) is set, it uses that explicit deployment; otherwise it runs `foundry-agent-setup.py` and lets the setup script auto-resolve the deployment.
...
- `missing Foundry deployment read permission`:
  ...
  - Grant the principal the required Foundry permissions if you want CI to auto-resolve a deployment when the agent is missing.
  - If the existing agent has been deleted and the workflow principal does not have Foundry `deployments/read`, provide `agent_model_deployment` (or `AZURE_AI_AGENT_MODEL_DEPLOYMENT`) so the workflow can call `foundry-agent-setup.py --model ...` without enumerating deployments.
